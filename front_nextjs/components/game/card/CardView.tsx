...
export default function CardView({ tiger, fox, rabbit, gam, state, gameSocket }) {
		...
		<div className="w-full h-full relative flex flex-row justify-center">
			{
				(tiger > 0 && state) && (
					<div className="flex flex-row w-[13rem] h-[16rem] mx-1 cardBox">
						...
						<Card name={`호랑이`} gameSocket={gameSocket}/>
					</div>)
			}
			{
				(fox > 0 && state) && (
					<div className="flex flex-row w-[13rem] h-[16rem] mx-1 cardBox">
						...
						<Card name={`여우`} gameSocket={gameSocket}/>
					</div>)
			}
			{
				(rabbit > 0 && state) && (
					<div className="flex flex-row w-[13rem] h-[16rem] mx-1 cardBox">
						...
						<Card name={`토끼`} gameSocket={gameSocket}/>
					</div>)
			}
			{
				(gam > 0 && state) && (
					<div className="flex flex-row w-[13rem] h-[16rem] mx-1 cardBox">
						...
						<Card name={`곶감`} gameSocket={gameSocket}/>
					</div>)
			}
			<style jsx>{`
			.cardBox:hover {
